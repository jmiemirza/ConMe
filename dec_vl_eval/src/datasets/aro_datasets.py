...
class VG_Relation(Dataset):
    def __init__(self, image_preprocess = None, text_perturb_fn=None, image_perturb_fn=None, root_dir=None, download=False):
        '''
...
class COCO_Order(Dataset):
    def __init__(self, args=None, image_preprocess=None, root_dir=None, max_words=30, split="test",
        ...
        for img_id, ann in tqdm(enumerate(self.annotation), total=len(self.annotation)):
            for i, caption in enumerate(ann['caption']):
                if len(self.dataset) > args.num_samples:
                    break
                # Construct a test case
                test_case = {}
                test_case["image_path"] = ann["image"]
                test_case["true_caption"] = pre_caption(caption, max_words)

                test_case["false_captions"] = []
                for perturb_fn in perturb_functions:
                    test_case["false_captions"].append(pre_caption(perturb_fn(caption), max_words))
    ...
    def __getitem__(self, index):  
        ...
        item = edict({"image_path": [image_path], "caption_options": test_case["caption_options"]})
        return item
...
class Flickr30k_Order(Dataset):
    def __init__(self, args=None, image_preprocess=None, split="val", root_dir=None, max_words=30, **kwargs):  
        """
        image_preprocess: image preprocessing function
        split: 'val' or 'test'
