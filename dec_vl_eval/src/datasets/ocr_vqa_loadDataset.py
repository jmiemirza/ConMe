...
################################################################
with open('ocr_vqa_dataset.json', 'r') as fp:
        data = json.load(fp)
...
if download == 1:
    if not os.path.exists('/dccstor/leonidka1/irenespace/data/ocr_vqa/images'):
        os.mkdir('/dccstor/leonidka1/irenespace/data/ocr_vqa/images')
    for k in data.keys():
        ext=os.path.splitext(data[k]['imageURL'])[1]
        outputFile='/dccstor/leonidka1/irenespace/data/ocr_vqa/images/%s%s'%(k,ext)
        # pdb.set_trace()
...
for k in data.keys():
    ext=os.path.splitext(data[k]['imageURL'])[1]
    imageFile='/dccstor/leonidka1/irenespace/data/ocr_vqa/images/%s%s'%(k,ext)
