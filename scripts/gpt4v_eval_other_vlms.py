...
partition = "replace_rel" # replace_att, replace_obj, replace_rel
num_samples = 333
experiment_name = f"{partition}_{num_samples}_filter-count-3"

# partition = "ln_subset"
# experiment_name = f"ln_subset"

select_by = "generate"
eval_dataset = 'GPT4V_FILTER'
output_root = f'/dccstor/leonidka1/irenespace/gpt4v_results/desc/step7/'

model = "internlm_xcomposer2_vl_7b"
# model = "idefics2-8b"
# model = 'gpt4v'
...
number_of_jobs = 1
mem = '80g'
gpu_type = 'a100' # '' or 'a100' # && hname!=cccxc451 && hname!=cccxc441'

# command related
command = f'export TRANSFORMERS_CACHE=/dccstor/leonidka1/irenespace/.cache; python -m prompt_negatives.vlm_eval_gpt4v --eval_dataset {eval_dataset} --dataset_partition {partition} --select_by {select_by} --model {model} --experiment_name {experiment_name} --job_name {model}'
