...
output_root = f'/dccstor/leonidka1/irenespace/gpt4v_results/desc/step2/'
# partition = "replace_rel"
# num_samples = 333
# experiment_name = f"{partition}_{num_samples}"
partition = f"ln_finetune" # ln_finetune, ln_less_nouns, ln_subset
...
# command related
command = f'export TRANSFORMERS_CACHE=/dccstor/leonidka1/irenespace/.cache; python -m prompt_negatives.vlm_gen_gpt4v --gpt4v_type desc --eval_dataset GPT4V_VLM_GEN --gpt4v_dataset_step 1 --dataset_partition {partition} --select_by generate --model {model} --model_path {model_path} --temperature 0 --max_new_tokens 500 --prompt 11 --experiment_name {experiment_name} --job_name {model}'
