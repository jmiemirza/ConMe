...
params_enum['eval_dataset'] = ['ed', 'SAMPLE_NEG_UPDATED'] # llava_mix or SAMPLE_NEG_UPDATED
params_enum['eval_vlm'] = ['ev', 'llava-v1.6-7b']
params_enum['dataset_partition'] = ['dp', 'replace_obj']
# params_enum['select_by'] = ['s', 'generate'] # generate or loss_score
# params_enum['num_samples'] = ['ns', 10]
...
# output_root = '/dccstor/leonidka1/irenespace/llm_results_updated/filter/llava-v1.5-7b/llava-mix/'
output_root = '/dccstor/leonidka1/irenespace/vlm_results/filter/llava-v1.6-7b/perp_and_gen/'
# output_root = '/dccstor/leonidka1/irenespace/llm_results_updated/filter/llava-v1.5-7b/mistral/t2i_and_gen/'
# output_root = '/dccstor/leonidka1/irenespace/sugar_results/corrected_templates/'
# job = time.strftime("%Y%m%d_%H%M")
# output_root = f"/dccstor/leonidka1/irenespace/prompt_negatives_revised/llava_mix/"

# fixed system params
is_multi_node = True
base_port = 10000
duration = '6h'
num_cores = 4 # originally 4
# num_workers = 4
num_nodes = 1 #if eval_only else 4
num_gpus = 1 if eval_only else 1
number_of_jobs = 1
mem = '80g' # originally 80g
gpu_type = 'a100' # a100_80gb >> a100 >> v100, && hname!=cccxc451 && hname!=cccxc441'
...
command = f'export TRANSFORMERS_CACHE=/dccstor/leonidka1/irenespace/.cache; {cmd_prefix} -m prompt_negatives.filter_negs_updated --prompt_model {prompt_model}'
