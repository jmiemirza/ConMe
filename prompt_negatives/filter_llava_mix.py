...
def prompt(
    ...
    for key, samples in data_dict.items():
        ...
        for ind, sample in enumerate(all_samples):
            ...
            print(f'Dataset item index {sample[-1]}\n')

            pos = sample[2]
            print('original positive: ', pos)

            alt = [text for text in sample[-2] if text != pos]
            if not alt:     # if empty, then the provided neg was actually identical to the positive
                continue
            neg = alt[0]    # otherwise, there should be exactly one negative in the present sample
            print('original negative: ', neg + '\n')

            # make sure image path exists
            image_path, question = sample[0], sample[1]
            ...
            # for saving file
            curr_dict = {
                'item_ind': sample[-1],  # original index from the loaded dataset
                'image': sample[0],  # original image path
                'orig_pos': pos,  # original positive
                'new_neg': neg,  # original negative
                'question': question # original "basic" question
            }
            ...
            if args.eval_vlm in {'llava-v1.5-7b', 'llava-v1.6-7b'}:
                for method in methods:
                    ...
                    if hit and reqs[f'{args.eval_vlm}_pass']:  # this means the VLM correctly chooses the ground truth caption i.e. negative is not hard enough
            ...
            # curr_choice = sorted(counts.keys(), key=lambda c: [counts[c], c])[0] # method with smaller count or alphabetically ordered if equal
