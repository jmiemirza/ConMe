...
def filter(
    ...
    new_rows = {vlm: [] for vlm in VLM_dict}
    total_samples = len(vlm_dfs['llava-v1.6-7b'])  # this length should be equal for all dfs
    ...
    for ind in range(total_samples):
        ...
        if include:
            for vlm, vlm_df in vlm_dfs.items():
                new_rows[vlm].append(vlm_df[ind:ind+1])

    for vlm, rows in new_rows.items():
        new_df = pd.concat(rows, ignore_index=True)
        new_df.to_csv(f'{args.output_path}/{args.step}/{args.job_name}/{vlm}/{args.select_by}/filter_results_dataframe_count-{args.filter_count}.csv')
